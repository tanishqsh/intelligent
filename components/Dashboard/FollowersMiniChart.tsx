...
const FollowersMiniChart = ({ data = [] }: { data: any[] }) => {
	...
	const followersData = data.find((d: any) => d.id === 'Followers');

	if (followersData) {
		followersData.data.forEach((p: any) => {
			if (p.y > max) {
				max = p.y;
			}
			if (p.y < min) {
	...
	const lineData = followersData
		? [
				{
					id: followersData.id,
					data: followersData.data.map((p: any) => ({
					...
					<div className="bg-white/10 backdrop-blur-sm px-4 py-1 shadow-sm border border-[#ededed]/30 rounded-md">
						<div className="space-x-1">
							<div className="text-xs rounded-full text-[#a5a5a5] px-[6px] py-[2px] bg-[#AAAAAA]/2 font-medium inline-block">
								{Number(follows)} followers <span className="opacity-50"> {dayjs(slice.points[0].data.x).fromNow()}</span>
							</div>
						</div>
					</div>
				);
